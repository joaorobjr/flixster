...
## Data exploration
The `flixster` tibble contains $\num{108779}$ unique users, who have rated at least one film.
The majority of the users rated just some films---$68.3\,\%$ of the users rated up to 10 films---while a small number of users rated over $\num{1000}$ movies---995 users in total, corresponding to approximately $0.9\,\%$ of the total users.
...
```{r heatmap, fig.align= "center", fig.height= 4, fig.width= 4, fig.cap= "\\label{heatmap}Heatmap of user *vs* movie created from a sample of unique user IDs. The matrix is sparse, with several empty cells. Some movies present more ratings, and some users are more active."}
...
Some of the titles are well-known blockbusters.
...
# Train/test split
We split the data set in two parts, in a 7-to-3 ratio: the *train* set, containing $70\,\%$ of the data, and the *test* set, with the remainder $30\,\%$.
The former was used to train the model, and we used the latter to simulate "fresh" data to evaluate the performance of the model.
```{r}
...
The *target* variable, *i.e.*, the one we want to predict, is `rating`.
So, it was removed from both the train and test sets, and saved in corresponding separate tibbles.
